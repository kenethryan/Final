...
def dashboard(request):
    ...
    unit_status_counts = {
        ...
        'on_board': units.filter(status='on_board').count(),
    ...
    context = {
        ...
        'recent_remittances': remittances[:5],  # Show last 5 remittances by default
        'start_date': start_date,
        'end_date': end_date,
...
def unit_rental_history(request, unit_id):
    ...
    data = [{
        ...
        'start_date': rental.date.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    if request.method == 'POST':
        try:
            ...
            if amount <= 0:
                ...
                messages.error(request, 'Withdrawal amount exceeds available savings.')
            else:
                driver.savings -= amount
                driver.save()
                messages.success(request, f'Successfully withdrew ₱{amount:,.2f} from savings.')
        ...
        try:
            with transaction.atomic():
                ...
                if cash_amount >= required:
                    ...
                    # Try to use extra fund to cover shortage
...
def accounts(request):
    ...
    active_units = Unit.objects.filter(status='on_board').count()
    on_board_units = Unit.objects.filter(status='on_board').count()
...
def toggle_driver_status(request, driver_id):
    ...
    if request.method == 'POST':
        driver.status = 'inactive' if driver.status == 'active' else 'active'
        driver.save()
...
def driver_transactions(request, driver_id):
    try:
        ...
        remittances = Remittance.objects.filter(driver=driver).order_by('date')  # Changed to ascending order
        for remit in remittances:
            ...
            transactions.append({
                ...
                'balance_at_time': None  # Will calculate after sorting
            })
...
def manage_damage_parts(request):
    ...
    available_units = Unit.objects.filter(status__in=['stand_by', 'on_board'])
...
def unit_statistics(request, unit_id):
    try:
        ...
        remittance_stats = {
            ...
            'last_remittance': remittances.order_by('-date').first().date.strftime("%Y-%m-%d") if remittances.exists() else None
        }
        ...
        maintenance_stats = {
            ...
            'avg_time_to_fix': maintenance_reports.filter(is_fixed=True).aggregate(
                avg_time=Avg(F('reported_date') - F('reported_date'))
            )['avg_time'] or None,
            'last_maintenance': maintenance_reports.order_by('-reported_date').first().reported_date.strftime("%Y-%m-%d") if maintenance_reports.exists() else None,
...
def driver_statistics(request, driver_id):
    ...
    try:
        ...
        total_units = units.count()
        current_unit = units.filter(status='on_board').first()  # Changed from 'active' to 'on_board' to match model choices
        
        # Calculate average time per unit
        unit_assignments = []
        for unit in units:
            if unit.updated_at:
                unit_assignments.append((timezone.now() - unit.updated_at).days)
        avg_time_per_unit = sum(unit_assignments) / len(unit_assignments) if unit_assignments else None
        ...
        return JsonResponse({
            ...
            'unit_stats': {
                ...
                'avg_time_per_unit': round(avg_time_per_unit, 1) if avg_time_per_unit else None,
                'status': driver.get_status_display()  # Use get_status_display() to get the human-readable status
            }
        })
    except Exception as e:
...
def assign_device(request):
    ...
    try:
        ...
        # Only allow superusers or staff to assign/remove devices
...
def unit_position_history(request, unit_id):
    ...
    try:
        ...
        # Get position history directly from Flespi API
...
def live_unit_positions(request):
    ...
    try:
        ...
        if not all_telemetry:
            logger.warning("No telemetry data returned from Flespi API")
            return JsonResponse({
                "success": False,
                "error": "No telemetry data available from the tracking service",
                "units": [],
                "mock_mode": False
            })
        ...
        unit_locations = []
        for unit in units:
            # Skip units without device IDs
            if not unit.flespi_device_id or unit.flespi_device_id not in all_telemetry:
                continue
                
            # Get device telemetry
            device_data = all_telemetry.get(unit.flespi_device_id, {})
            ...
            if 'latitude' in device_data:
                # Simplified format returned by our get_all_devices_telemetry method
                ...
                # Native Flespi telemetry format with position.latitude.value
...
def get_live_unit_position(request, unit_id):
    ...
    try:
        ...
        if not telemetry or telemetry.get("latitude") is None or telemetry.get("longitude") is None:
...
def flespi_api_diagnostics(request):
    ...
    about any issues. Only accessible to superusers.
    """
...
def assign_driver(request):
    ...
    try:
        ...
        if Unit.objects.filter(driver=driver, status='on_board').exists():
            return JsonResponse({
                'success': False,
